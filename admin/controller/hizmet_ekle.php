    ...
    if( $user["access"]["hizmetler"] != 1  ):
        ...
        if ( $_FILES["logo"] && ( $_FILES["logo"]["type"] == "image/jpeg" || $_FILES["logo"]["type"] == "image/jpg" || $_FILES["logo"]["type"] == "image/png" || $_FILES["logo"]["type"] == "image/gif"  ) ):
          ...
          $permalink = permalink($page_name);
            ...
            $insert->execute(array("page_name"=>$page_name,"page_keywords"=>$page_keywords,"page_content"=>$page_content,"page_description"=>$page_description,"page_image"=>$logo_newname,"page_get"=>$permalink,"type"=>1,"date"=>date("Y.m.d H:i:s") ));
            
                        
            if( $insert ){ 
                $id = $conn->lastInsertId(); 
                header("Location: ".site_url("admin/hizmet_detay/$id"));
            }

        endif;
       

  require admin_view('hizmet_ekle');